...
class TripService {
  ...
  /// Live version of [myTrips], for the dashboard's trip-reminder banner —
  /// re-emits whenever the signed-in user's trip memberships change.
  /// Streams `trip_members` (cheap, keyed to the user) and re-fetches the
  /// matching `trips` rows on every change, rather than streaming `trips`
  /// directly, since RLS on `trips` can't filter by membership inside a
  /// realtime stream.
