...
class CommunityPost {
  const CommunityPost({
    ...
    this.ipAddress,
    this.locationName,
    this.authorLocationSharingEnabled = true,
  });
  ...
  /// The poster's IP address, captured once at post time (see
  /// `CommunityService.addPost`) — always stored regardless of the
  /// author's Location Sharing setting; only [ipDisplay] hides it.
  final String? ipAddress;

  /// "City, District" for the poster's GPS position at post time (e.g.
  /// "George Town, Bayan Lepas"), reverse-geocoded via Photon — see
  /// `CommunityService.addPost`. `null` if location wasn't available
  /// (permission denied, services off, or the lookup failed).
  final String? locationName;

  /// The post author's *current* Settings → Privacy & Security →
  /// "Location Sharing" value (see `ProfileService.setLocationSharingEnabled`)
  /// — read live at fetch time, not frozen from when the post was made, so
  /// toggling that setting immediately changes what already-published posts
  /// show.
  final bool authorLocationSharingEnabled;

  /// What `PostCard` renders after "IP: " — the captured location name
  /// (never the raw IP digits, even though [ipAddress] is still stored)
  /// only while the author currently has Location Sharing on; "Unknown"
  /// otherwise (off, or the location failed to capture at post time).
  String get ipDisplay =>
      authorLocationSharingEnabled ? (locationName ?? 'Unknown') : 'Unknown';
